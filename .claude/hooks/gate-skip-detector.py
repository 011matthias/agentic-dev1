...
Fires advisories when patterns suggest a gate was skipped:
  ...
  (a) git push / gh pr merge with no prior validation run (tools/verify.py,
      a product build, or tests) -> pre-publish validation gate skipped
  (b) Same fix attempted 3+ times in buffer -> iteration-3x gate
...
VALIDATE_PATTERNS = [
    ...
    r"\bpytest\b",
    r"\buv\s+run\s+(pytest|tools/(verify|validate))",
    r"\btsc\b",
...
READONLY_PATTERNS = [
    ...
    r"\btools/verify\.py\b",                 # verify.py re-runs are not a fix loop
    ...
    if pub_scan and any(re.search(p, pub_scan) for p in PUBLISH_PATTERNS):
        ...
        if not had_validate:
            ...
            advisories.append(
                ...
                "(git push / gh pr merge) with no validation step in the recent "
                "buffer. Recommended: run `uv run tools/verify.py <scope>` (or the "
                "product build / tests) BEFORE publishing. See rule_behaviors.md "
                "B2 gate. friction-event:gate-skip-pre-publish"
            )
