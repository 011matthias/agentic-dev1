...
| skil_website-quality | web/website | frontend-design (vendored) covers the visual half (distinctiveness, type, motion, copy) and the surveyed SEO/a11y candidates are generic advice the base model already applies. None codify the dev1 web-platform bar as a reviewable rubric: the exact website verify-profile gate (Lighthouse perf >= 90 / a11y >= 95, link + meta/OG presence from rule_testing) plus the agent-judged dimensions a presence-check cannot (SEO substance, share-preview correctness, programmatic a11y a screenshot hides, Core Web Vitals felt, content not-AI-generic). Authored as the website twin of skil_game-feel-review: archetype-keyed and product-agnostic, deliberately NOT the product-bound skil_crew-verify shape. |
| skil_website-design | web/website | frontend-design (vendored) supplies the taste REASONING but not the dev1 PROCESS: it does not name the loop that produces a per-project direction plan, write it into THIS infrastructure (contract.css / brand.css / the Fonts API pairing / the primitives), or wire the "would I produce this for any other brief?" critique to the website-reference.md tell-list and design-reviewer. Authored as the design twin of skil_website-quality (this DECIDES identity; quality VERIFIES adequacy), archetype-keyed and product-agnostic. Operationalizes frontend-design rather than duplicating it. |
...
| 4 | write | skil_crew-verify | DONE 2026-06-16. `products/crew/verify/round.py` drives a real headless Confessions round (webapp-testing approach, inline dev server) and asserts the rule_code_review invariants from the logged events: Confessions anonymity (no per-player vector reaches `crew_logs_v1`), no measurement double-fire (exact event counts), schema, plus static tripwires (StrictMode absent, press-squash CSS, keys + snapshot `v`, event vocabulary). Wired as products/crew npm `verify` (verify.py auto-discovers it); runtime tier SKIPs cleanly without a browser so the bar stays portable. Each substantive assertion mutation-tested to bite. Closes the Cycle-4 anneal (Confessions anonymity had no test). SKILL.md is the recipe. |
| 5 | write | skil_pwa-install-and-measure | DEFERRED; lower priority, build when the install/measurement work is scheduled. |
| 7 | write | skil_website-design | DONE 2026-06-17 (website infrastructure, piece 2). The PROCESS skill: a four-pass loop (brainstorm -> critique-against-defaults -> build -> critique-again) that turns a brief into a per-project direction plan (palette / pairing / layout / one signature) and writes it into the token layer (brand.css + experimental.fonts). The "would I produce this for any other brief?" critique is the mandatory gate against convergence on a house look. Design twin of skil_website-quality (decides identity vs verifies adequacy); operationalizes the vendored frontend-design rather than duplicating it; pairs with design-reviewer + docs/website-reference.md (the tell-list). |
| 6 | write | skil_website-quality | DONE 2026-06-16 (Cycle 8). Archetype-keyed, product-agnostic web-platform quality bar: the mechanical website verify gate (Lighthouse perf >= 90 / a11y >= 95, link + meta/OG presence) plus a 6-dimension agent-judged rubric (SEO substance, share preview, programmatic a11y, CWV felt, content, responsive floor). The website twin of skil_game-feel-review; visual taste routed to design-reviewer + frontend-design. Mechanical tier wires into templates/website's npm verify when that archetype is filled from the Astro harvest. |
